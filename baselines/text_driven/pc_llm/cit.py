...
def CIT(data, method='fisherz', **kwargs):
    ...
    if method == fisherz:
        ...
        return LLMBasedConditionalIndependece(data, **kwargs)
    else:
...
class LLMBasedConditionalIndependece(CIT_Base):
    ...
    def __init___(
        ...
        super().__init__(data, **kwargs)
        self.kwargs = kwargs
        self.check_cache_method_consistent('llm_ci', NO_SPECIFIED_PARAMETERS_MSG)
        self.assert_input_data_is_valid()
    ...
    async def __call__(self, X: str, Y: str, condition_set: List[str] = []) -> float:
        # load cache
        cache = json.load(open('baselines/text_driven/pc_llm/cache.json', 'r'))
        ...
        n_trials = 1
        null = 'yes'
        expert = UncertainExpert(model=model, temperature=temperature)
        Xs, Ys, condition_set, cache_key = self.get_formatted_XYZ_and_cachekey(X, Y, condition_set)
        if cache_key in self.pvalue_cache: return self.pvalue_cache[cache_key]
        no_count, yes_count = 0, 0
        for i in range(n_trials):
            reply = await expert.independence_test(
                X,
                Y, 
                condition_set,
                vars=dataset.var_description)
            if reply == 1:
                yes_count += 1
            else:
                no_count += 1
        print(yes_count)
        return yes_count
        # p_no, p_yes = no_count / n_trials, yes_count / n_trials
        # p = (no_count + yes_count) / (n_trials)
        # se = np.sqrt(p * (1 - p) / n_trials)

        # if null == 'yes':
        #     pval = norm.sf((p_no - p_yes) / se)
        # else:
        #     pval = norm.sf((p_yes - p_no) / se)

        # print(pval)
        # self.pvalue_cache[cache_key] = pval
        # return pval
